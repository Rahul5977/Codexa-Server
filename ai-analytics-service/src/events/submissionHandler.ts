...
export const handleSubmissionCompleted = async (
  ...
  // Only run if we have the code and Gemini is configured
  // ------------------------------------------------------------------
  if (data.code && env.GEMINI_API_KEY) {
    generateAIAnalysis({
      ...
      // Non-fatal: just log the failure
      console.warn(
        `⚠️  Background AI analysis failed for submission ${data.submissionId}: ${err.message}`,
      );
    });
  }
};
